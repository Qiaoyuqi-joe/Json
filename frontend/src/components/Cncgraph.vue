<script>
/*此文件用于生成算力网络服务卸载图，使用j3力向导图将节点按照集群聚类展示，并用不同颜色区分。
将基站（8个）建模为节点并将基站之间的卸载决策建模为边，初始时隐藏不同节点间卸载决策，节点被选中时
展示与节点相关的卸载决策并浮窗显示节点信息，节点信息包括被分配/已使用的CPU资源和被分配/已使用的内存资源
点击按钮可以选择展示完整算力网络图或只展示节点。
用户使用selector选择需要查看的服务（共4个）,选择成功后展示该服务对应的卸载决策
*/
...
/*             node3:{ "name": "node8", "cpu_allocation": "311 mcores", "mem_allocation": "211MB", "cpu_used": "211 mcores", "mem_used": "111MB" },*/
    ...
    methods: {
        ...
        showTooltip(content, x, y) { //点击节点时展示浮窗
        ...
        showClusterInfo(cluster) { //显示集群信息，将基站节点信息存储至对应集群的结构体中
            const cluster1 = { name: "Cluster A", nodes: [] };
            const cluster2 = { name: "Cluster B", nodes: [] };
            this.nodes.forEach(node => {
                if (node.cluster == '1') { cluster1.nodes.push(node);
                } else {
        ...
        importdata(){//将从store中读到的数据存储为力向导图需要的格式
            this.nodes = [];
            this.links = [];
